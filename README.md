...
パディングとは，入力画像の周囲に画素を追加する操作であり，本モデルではゼロパディング(囲う画素値が0)を使用し,二周分画像の周囲を囲います．
出力チャンネル数を 6で， $k$ 番目のチャンネルにおける
...
* (u, v): カーネルの座標系における画素の位置
* $w^{(1)}_k(u, v)$ : 重み
* x(i - 1 + u, j - 1 + v): 画素値
* $b^{(1)}_k$ : バイアス 

* $z^{(1)}_k$ : kチャンネル目の畳み込み計算後の出力特徴マップ

* $z^{(1)}_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素


次式に示すように畳み込みの結果 $z^{(1)}_k(i, j)$ に対して活性化関数であるReLU関数を使用します．

$$
h^{(1)}_k(i, j) = max(0, z^{(1)}_k(i, j))
$$

ここで，

* $h^{(1)}_k$ : kチャンネル目のReLU関数計算後の出力特徴マップ
...
具体的には，ReLU関数計算後の出力 $h^{(1)}$ の各チャンネルに対して次式を計算します．

$$
p^{(1)}_k(i, j) = max_{n, m \in W(i, j)} h^{(1)}_k(n, m)
$$

ここで
* $p^{(1)}_k$ : kチャンネル目のmax poolng後の出力特徴マップ

* $p^{(1)}_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素

* $W(i, j)$ : 入力特徴マップ $h$ において，左上の座標 $(i \cdot s, j \cdot s)$ を起点とするプーリング窓内に含まれるインデックス $(n, m)$ の集合．このsはストライドの移動量で本モデルでは $s = 2$

#### 第2畳み込み層

第1プーリング層の出力特徴マップ $\mathbf{p^{(1)}}$ を入力として，6 チャンネルに対する $5 \times 5$ カーネルを用いた畳み込み演算をおこなう．
本層ではパディングは使用しない．
出力チャンネル数を 16 とし，$k$ 番目の出力チャンネルにおける特徴マップ上の位置 $(i, j)$ の出力値は次式で与えられる．

$$
z^{(2)}_k(i, j) = \sum_{c=1}^{6} \sum_{u=1}^{5} \sum_{v=1}^{5} w_{k,c}(u, v) p^{(1)}{c}(i - 1 + u, j - 1 + v) + b_k
$$
...
* $(u, v)$：畳み込みカーネル内の画素位置  
* $p_{1,c}(i - 1 + u, j - 1 + v)$：第1プーリング層の $c$ 番目チャンネルにおける入力画素値  
* $w_{k,c}(u, v)$：出力チャネル $k$，入力チャネル $c$ に対応する畳み込みカーネルの重み  
* $b_k$：出力チャネル $k$ に対応するバイアス  
* $z^{(2)}_k(i, j)$：第2畳み込み層における出力  

得られた出力に対して ReLU 関数を適用し，
...
$$
p1_k(i, j) = max_{n, m \in W(i, j)} h1_k(n, m)
$$

ここで
* $p1_k$ : kチャンネル目のmax poolng後の出力特徴マップ

* $p1_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素
...
#### 全結合層＋ReLU
全結合層に入力するために，畳み込み層やプーリング層を経て得られた特徴マップを，空間次元およびチャンネル次元を含めて一次元のベクトル $\mathbf{f}$ に変換します．
