...
#### 第1全結合層＋ReLU
全結合層に入力するために，第2プーリング層の出力である特徴マップ $p^{(2)} \in \mathbb{R}^{16\times5\times5}$ の全要素を並べ替え，1次元ベクトル $\boldsymbol{f}$ に変換（flatten）します．このとき， $\boldsymbol{f}\in\mathbb{R}^{400}$ となります．
...
#### 第2全結合層＋ReLU
次式に示すように，第1畳み込み層とReLU関数を経た出力 $\boldsymbol{h}^{(3)}$ を第2全結合層に入力して84次元のベクトルを出力します．
...
最終全結合層を経て出力された $\boldsymbol{z}^{(5)}$ はlogitsと呼ばれ，モデルの学習時に後述するSoftmax関数で確率化する前のクラスごとにおけるスコアとなっています．
...
正解ラベルを one-hot 表現を $\boldsymbol{y}^*$ とすると，
交差エントロピー損失は次式で表されます．
