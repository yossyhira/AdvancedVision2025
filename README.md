...
#### 第1全結合層＋ReLU
全結合層に入力するために，第2プーリング層の出力である特徴マップ $p^{(2)} \in \mathbb{R}^{16\times5\times5}$ の全要素を並べ替え，400 個の成分を持つベクトル  $\boldsymbol{f}$ に変換（flatten）します．このとき， $\boldsymbol{f}\in\mathbb{R}^{400}$ の列ベクトルとなります．また，これ以降 $\mathbb{R}^{n}$ と表記した場合は，列ベクトル(n行1列)と定義します．
