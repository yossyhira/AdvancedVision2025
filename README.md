...
#### 畳み込み層＋ReLU
入力画像 $\mathbf{x}$ に対してn*nのカーネルを用いた畳み込み演算をおこないます．
出力チャンネルは $\mathbf{K}$ チャンネルあるとし，k番目のチャンネルの特徴マップにおける位置(i, j)の出力値は 次式で表すことができます．

$$
z_k(i, j) = \sum_{u=1}^{n}\sum_{v=1}^{n} w_k(x, y) x(i - 1 + u, j - 1 + v) + b_k
$$

ここで，

* (i, j): 畳み込み後の出力における画素の位置
* (u, v): フィルタの座標系における画素の位置
* $w_k(u, v)$ : 重み
* x(i - 1 + u, j - 1 + v): 画素値
* $b_k$ : バイアス 
* $z_k(i, j)$ : k番目のチャンネルの特徴マップにおける出力値


次式に示すように畳み込みの各結果 $z_k(i, j)$ に対して活性化関数であるReLU関数を使用します．
...
ここで，
* h_k(i, j): ReLU関数の計算後におけるk番目のチャンネルの特徴マップ出力値
