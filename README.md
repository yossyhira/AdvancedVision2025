...
したがって，パディング後の入力画像は $\tilde{\mathbf{x}} \in \mathbb{R}^{1 \times (28 + 4) \times (28 + 4)}$ となります．
...
#### 最終層(全結合層＋Softmax)
次式に示すように，第2全結合層とReLU関数を経た出力 $\mathbf{h}^{(4)}$ を最終全結合層に入力して10次元のベクトルを出力します．
...
最終全結合層を経て出力された $\mathbf{z}^{(5)}$ はlogitsと呼ばれ，モデルの学習時に後述するSoftmax関数で確率化する前のクラスごとにおけるスコアとなっています．
また，実装では学習済みモデルで分類時にSoftmax関数を通さずlogitsのスコアが最も高いクラスを予測結果としています．理由は，Softmaxを通すか否かにかかわらず選ばれるクラスは同じだからです．

最終全結合層で出力されたlogitsを学習時には確率に直す必要があるので，次式で示すSoftmax関数を適用します．
...
* $i$：i個目のクラス
* $z^{(5)}_i$：最終全結合層の出力 $\mathbf{z}^{(5)}$ におけるクラス $i$ の logits  
* $z^{(5)}_u$：最終全結合層の出力 $\mathbf{z}^{(5)}$ におけるクラス $u$ の logits  
* $\mathbf{y}\in\mathbb{R}^{10}$：Softmax 関数によって得られる，各クラスに対する予測確率ベクトル
* $y_i$：入力画像がクラス $i$ に属する予測確率  

なお，実装では PyTorch の仕様により，後述する交差エントロピー損失関数の内部で
Softmax 関数が暗黙的に適用されるため，モデル内で明示的に Softmax を計算していません．

#### 損失関数
本モデルは Softmax 関数によりクラスごとの確率分布を出力する多クラス分類問題であるため，
損失関数として交差エントロピー損失を使用します．
正解ラベルを one-hot 表現を $\mathbf{y}^*$ とすると，
交差エントロピー損失は次式で表されます．
