...
$$
z1_k(i, j) = \sum_{u=1}^{5}\sum_{v=1}^{5} w1_k(u, v) x(i - 1 + u, j - 1 + v) + b1_k
$$
...
* $z1_k$ : kチャンネル目の畳み込み計算後の出力特徴マップ

* $z1_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素


次式に示すように畳み込みの結果 $z_k(i, j)$ に対して活性化関数であるReLU関数を使用します．

$$
h1_k(i, j) = max(0, z1_k(i, j))
$$

ここで，

* $h1_k$ : kチャンネル目のReLU関数計算後の出力特徴マップ

* $h1_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素
...
#### 第二畳み込み層＋ReLU
プーリング後の出力特徴マップ $\mathbf{p1}$ の各チャンネルを入力として $5 \times 5$ のカーネルを用いた畳み込み演算をおこないます．
ここではパディングは使用しない．
