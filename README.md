...
* $z^{(1)}_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素値
...
* $h^{(1)}_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素値
...
* $p^{(1)}_k(i, j)$ : 出力特徴マップ上の位置 (i,j) における要素値
...
* $(i, j)$：畳み込み後の出力特徴マップにおける要素の位置  
* $(u, v)$：畳み込みカーネルの座標系における要素の位置
* $p^{(1)}_c(i - 1 + u, j - 1 + v)$：第1プーリング層の $c$ 番目チャンネルにおける入力要素値  
...
* $z^{(2)}_k(i, j)$：出力特徴マップにおける位置 (i,j) の要素値
...
* $h^{(2)}_k(i, j)$ : 出力特徴マップにおける位置 (i,j) の要素値
...
* $p^{(2)}_k(i, j)$：出力特徴マップ上の位置 $(i, j)$ における要素値
* $a, b$ :プーリング窓内の位置を示すインデックス  
* $s$：ストライド
...
最終全結合層を経て出力された $\boldsymbol{z}^{(5)}$ の各成分はlogitsと呼ばれ，後述するSoftmax関数で確率化する前のクラスごとにおけるスコアとなっています．
